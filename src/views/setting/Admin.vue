<template>
  ...
  <!--添加-->
  <a-button type="primary" class="mt-4" v-if="userActions.add&&roleActions.list&&tenantActions.list"
            @click="showAddModal">
    <template #icon>
      <PlusCircleOutlined/>
    </template>
  ...
  <UserModal :visible="userModal" @handleUser="handleUser"
             @cancel="handleCancel" :tenants="tenants"
...
import {addUser, getAdminList, getTenantList, updateAdmin, updateAdminStatus} from '@/api/setting'
...
export default defineComponent({
  ...
  setup() {
    ...
    const userList = () => {
      getAdminList({current: data.current, page_size: data.pageSize}, username.value).then(res => {
        data.loading = false
    ...
    const handleUpdateUserStatus = (record: any) => {
      updateAdminStatus(record.id).then(() => {
